...
for conf in pars['datasets']:
    ...
    if (args.justWeights): # generating only weight distribution
        ...
        if 'ZHTollSS' not in data_name:
            os.makedirs(mk_dir, exist_ok=True)
        else:
            os.makedirs(cwd+'/'+args.outdir+'/'+'ZH', exist_ok=True)
    ...
    for i in range(1, nmax+1, njobs):
        ...
        if (args.justWeights): # generating only weight distribution
            ...
            if 'ZHTollSS' in out_name:
                line = conf + "," + args.cuts + "," +args.outdir+"/"+"ZH"+"/output_"+out_name + ".root" + "," + start + "," + end + "," + data_name + ','+ args.year
            else:
